...
class HomePage:
    ...
    def __init__(self,  screen: pygame.display,
                 title: str = const.TITLE,
                 crosses_player: int = 0,
                 zeros_player: int = 1) -> None:
        """
...
# ------------------------CОЗДАНИЕ--------------------------------------------------
    ...
    def create_home_page(self) -> None:
        ...
        :return:
        """
        self.objs_on_the_screen.append(pygame.image.load(const.START_IMAGE_PATH))
    ...
    def create_game_page(self, size: int = 3, pieces_to_win: int = 3) -> None:
        ...
        :return:
        """
        classic_game_page = GamePage(self, size=size,
                                     pieces_to_win=pieces_to_win,
                                     crosses_player=self.crosses_player,
                                     zeros_player=self.zeros_player)
    ...
    def create_group_of_buttons(self) -> None:
        ...
        :return:
        """
        buttons = [Button(self.open_game_page, hitbox=const.BUTTON_THREE_HITBOX,
                          inner_text=const.BUTTON_THREE_CONTENT, default_texture=const.BUTTON_DEFAULT_DEFAULT_TEXTURE,
                          hover_texture=const.BUTTON_DEFAULT_HOVER_TEXTURE,
                          click_texture=const.BUTTON_DEFAULT_CLICK_TEXTURE, border_radius=const.BUTTON_BACK_BOARDER_RADIUS),
                   Button(self.open_game_page, 4, 3, hitbox=const.BUTTON_FOUR_HITBOX,
                          inner_text=const.BUTTON_FOUR_CONTENT, default_texture=const.BUTTON_DEFAULT_DEFAULT_TEXTURE,
                          hover_texture=const.BUTTON_DEFAULT_HOVER_TEXTURE,
                          click_texture=const.BUTTON_DEFAULT_CLICK_TEXTURE, border_radius=const.BUTTON_BACK_BOARDER_RADIUS)]
        ...
        self.objs_on_the_screen.append(group_of_buttons)
    ...
    def create_generate_button(self) -> None:
        ...
        size_text = StylizedText(const.NUMBERFIELD_FOR_SIZE_HITBOX, const.NUMBERFIELD_FOR_SIZE_CONTENT,
                                 const.NUMBERFIELD_FOR_SIZE_COLOR, const.NUMBERFIELD_FOR_SIZE_FONT_FAMILY,
                                 const.NUMBERFIELD_FOR_SIZE_FONT_SIZE)
        win_text = StylizedText(const.NUMBERFIELD_FOR_WIN_HITBOX, const.NUMBERFIELD_FOR_WIN_CONTENT,
                                const.NUMBERFIELD_FOR_WIN_COLOR, const.NUMBERFIELD_FOR_WIN_FONT_FAMILY,
                                const.NUMBERFIELD_FOR_WIN_FONT_SIZE)
        ...
        win_block = NumberField(const.WIN_BLOCK_HITBOX, background_text=win_text,
                                background_texture=const.SCREEN_COLOR)
        self.objs_on_the_screen.append(size_block)
        self.objs_on_the_screen.append(win_block)
        generative_button = Button(self.user_game, size_block, win_block,
                                   hitbox=const.BUTTON_GENERATE_HITBOX,
                                   inner_text=const.BUTTON_GENERATE_CONTENT,
                                   default_texture=const.BUTTON_GENERATE_DEFAULT_TEXTURE,
                                   hover_texture=const.BUTTON_GENERATE_HOVER_TEXTURE,
                                   click_texture=const.BUTTON_GENERATE_CLICK_TEXTURE,
                                   border_radius=const.BUTTON_GENERATE_BOARDER_RADIUS)
        self.objs_on_the_screen.append(generative_button)
...
# ------------------------РЭНДЕР-----------------------------------------------------
    def render(self) -> None:
        ...
        :return: None
        """
        for obj in self.objs_on_the_screen:
            if isinstance(obj, pygame.Surface):
                self.screen.blit(
                    obj, const.START_IMAGE_COORDS)
            else:
...
# ---------------------------------ОБРАБОТКА ДЕЙСТВИЙ-------------------------------------------
    ...
    def user_game(self, size_field: NumberField, win_field: NumberField) -> None:
        ...
        :return:
        """
        if (size_field.is_text_correct and win_field.is_text_correct and
                int(win_field.text.content) <= int(size_field.text.content)):
    ...
    def __repr__(self) -> str:
        return (f'HomePage(screen={self.screen}, title={self.title}, '
                f'crosses_player={self.crosses_player}, '
                f'zeros_player={self.zeros_player})')
...
class GamePage:
    ...
    def __init__(self, home_page: HomePage, 
        ...
        self.agent = TicTacToeAgent.load(const.MODEL_PATH)
        self.visual_board: list[Button] = []
        self.home_page: HomePage = home_page
        self.objs_on_the_screen: dict = {}
        self.create_game_page()
...
# ------------------------ СОЗДАНИЕ СТРАНИЦЫ С ИГРОЙ ---------------------------------
    ...
    def create_buttons(self) -> None:
        ...
        :return:
        """
        buttons = [Button(self.open_home_page, hitbox=const.BUTTON_BACK_HITBOX, inner_text=const.BUTTON_BACK_CONTENT,
                          default_texture=const.BUTTON_BACK_DEFAULT_TEXTURE,
                          hover_texture=const.BUTTON_BACK_HOVER_TEXTURE, click_texture=const.BUTTON_BACK_CLICK_TEXTURE,
                          border_radius=const.BUTTON_BACK_BOARDER_RADIUS),
                   Button(self.restart_game, hitbox=const.BUTTON_RESET_HITBOX, inner_text=const.BUTTON_RESET_CONTENT,
                          default_texture=const.BUTTON_RESET_DEFAULT_TEXTURE,
                          hover_texture=const.BUTTON_RESET_HOVER_TEXTURE, click_texture=const.BUTTON_RESET_CLICK_TEXTURE,
                          border_radius=const.BUTTON_RESET_BOARDER_RADIUS),
                   Button(self.move_back, hitbox=const.BUTTON_UNDO_HITBOX,
                          inner_text=const.BUTTON_UNDO_CONTENT, default_texture=const.BUTTON_UNDO_DEFAULT_TEXTURE,
                          hover_texture=const.BUTTON_UNDO_HOVER_TEXTURE, click_texture=const.BUTTON_UNDO_CLICK_TEXTURE,
                          border_radius=const.BUTTON_BACK_BOARDER_RADIUS)]
        group_of_buttons = GroupObjectClass(
            buttons, const.GAME_GROUP_OF_BUTTONS_HITBOX, 'horizontal', const.GAME_GROUP_OF_BUTTONS_MARGINS)
        self.objs_on_the_screen['groupOfButtons'] = group_of_buttons
        ...
        for y in range(self.size):
            for x in range(self.size):
                ...
                if self.crosses_player == self.zeros_player == 1:
                    ...
                    self.objs_on_the_screen[number_of_button] = board_cell
                else:
                    board_cell = ClickableCell(hitbox, 
                                               self.make_move, 
                                               number_of_button, 
                                               crosses, zeros, 
                                               default_texture=pygame.color.Color(const.SCREEN_COLOR), 
                                               hover_texture=const.SCREEN_COLOR)
                    self.objs_on_the_screen[number_of_button] = board_cell
                self.visual_board.append(
                    self.objs_on_the_screen[number_of_button])
    ...
    def create_sticks(self) -> None:
        ...
        for i in range(1, self.size):
            ...
            self.objs_on_the_screen[f'vert{i}'] = pygame.Rect(coords_vertical)
            ...
            self.objs_on_the_screen[f'hor{i}'] = pygame.Rect(coords_horizontal)
...
# -------------------------- Рэндер страницы с классической игрой -----------------
    def render(self) -> None:
        ...
        :return:
        """
        for obj in self.objs_on_the_screen.values():
            if isinstance(obj, pygame.Rect):
                pygame.draw.rect(self.home_page.screen, const.INNER_BOARDERS_COLOR, obj)
            else:
                obj.render(self.home_page.screen)
...
# ----------------------- РАБОТА С ДЕЙСТВИЯМИ НА СТРАНИЦЕ ----------------------
    ...
    def move_back(self) -> None:
        ...
        """
        if ((len(self.moves) > 0
                and not (self.crosses_player == 1 and self.zeros_player == 0))
                or self.crosses_player == 1
                and self.zeros_player == 0
        ...
        if not self.is_game_continue():  # Если игра закончилась, то ход делать не надо
            ...
            match zeros_player:  # Определение того, кто играет за ноликов
                ...
                case 2:
                    ...
                    self.zeros_move(computers_turn)
            if crosses_player > 0:  # если противник компъютер, то вызываем его ход
            ...
            match crosses_player:
                ...
                case 2:
                    computers_turn = self.agent.predict(self.logic.board.get_uid(), self.logic.find_legal_moves())[1]
                    self.zeros_move(computers_turn)
            if zeros_player > 0:  # если противник компъютер, то вызываем его ход
                self.make_move(crosses_player=crosses_player,
                               zeros_player=zeros_player)
...
def main():
    start = HomePage(pygame.display.set_mode((880, 880)),
                     crosses_player=0, zeros_player=2)
    start.run()
